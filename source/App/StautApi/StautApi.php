 ...
 */
class StautApi extends Controller
{
    ...
    public function __construct()
    {
        ...
        $request = $this->requestLimit("stautApi", 1, 1);
        if (!$request) {
            exit;
        }

        if ($_REQUEST["route"] == "/users" && !empty($_POST)) {
            // No Auth
        } else {
            $auth = $this->auth();
            if (!$auth) {
     ...
     * @return StautApi               [description]
     */
    protected function call(int $code, string $type = null, string $message = null, string $rule = "errors"): StautApi
    {
     ...
     * @return StautApi
     */
    protected function back(array $response = null): StautApi
    {
        if (!empty($response)) {
    ...
    private function auth(): bool
    {
        ...
        if (empty($this->headers["email"]) || empty($this->headers["password"])) {
            $this->call(
                400,
                "auth_empty",
                "Favor informar seu e-mail e senha"
            )->back();
            return false;
        }
    ...
    /**
     * [requestLimit description]
     * @param  string       $endpoint [description]
     * @param  int          $limit    [description]
     * @param  int          $seconds  [description]
     * @param  bool|boolean $attempt  [description]
